from flask import Flask, render_template, send_from_directory, jsonify
import os
from datetime import datetime
import json

app = Flask(__name__)

# Конфигурация
DOWNLOAD_DIR = 'static/downloads'

# Список игр и их конфигурация
GAMES_CONFIG = {
    'ddnet': {
        'name': 'DDNet',
        'description': 'DDNet - это модификация Teeworlds с улучшенной физикой, новыми режимами игры и множеством карт.',
        'icon': 'ddnet.png'
    },
    'cs2': {
        'name': 'CS 2',
        'description': 'Counter-Strike 2 - современная версия легендарного шутера от Valve с обновленной графикой и механиками.',
        'icon': 'cs2.png'
    },
    'minecraft': {
        'name': 'Minecraft',
        'description': 'Minecraft - песочница с бесконечными возможностями: стройте, исследуйте, выживайте или творите.',
        'icon': 'minecraft.png'
    }
}

@app.route('/')
def index():
    """Главная страница"""
    return render_template('index.html')

@app.route('/api/files/<game>')
def get_files(game):
    """API для получения списка файлов для игры"""
    if game not in GAMES_CONFIG:
        return jsonify({'error': 'Game not found'}), 404

    game_dir = os.path.join(DOWNLOAD_DIR, game)
    if not os.path.exists(game_dir):
        return jsonify({'files': []})

    files = []
    for filename in os.listdir(game_dir):
        filepath = os.path.join(game_dir, filename)
        if os.path.isfile(filepath):
            # Получаем информацию о файле
            stat = os.stat(filepath)
            files.append({
                'name': filename,
                'size': stat.st_size,
                'size_human': format_size(stat.st_size),
                'modified': datetime.fromtimestamp(stat.st_mtime).strftime('%Y-%m-%d %H:%M'),
                'url': f'/download/{game}/{filename}'
            })

    # Сортируем по дате модификации (новые сверху)
    files.sort(key=lambda x: x['modified'], reverse=True)
    return jsonify({'files': files})

@app.route('/download/<game>/<filename>')
def download_file(game, filename):
    """Скачивание файла"""
    if game not in GAMES_CONFIG:
        return 'Game not found', 404

    return send_from_directory(os.path.join(DOWNLOAD_DIR, game), filename, as_attachment=True)

@app.route('/api/games')
def get_games():
    """API для получения конфигурации игр"""
    return jsonify(GAMES_CONFIG)

def format_size(bytes_size):
    """Форматирование размера файла в человеко-читаемый вид"""
    for unit in ['B', 'KB', 'MB', 'GB']:
        if bytes_size < 1024.0:
            return f"{bytes_size:.1f} {unit}"
        bytes_size /= 1024.0
    return f"{bytes_size:.1f} TB"

if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000)